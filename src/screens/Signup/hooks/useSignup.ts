...
export const useSignup = (): UseSignup => {
  ...
  const changeFieldValue = React.useCallback((event: React.FormEvent<HTMLInputElement>) => {
  ...
  const signup = React.useCallback(async (event: React.FormEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      if (isCreated) {
        ...
        const authState: AuthState = res2.status === 200 ? 'AUTH' : 'NOT-AUTH';
